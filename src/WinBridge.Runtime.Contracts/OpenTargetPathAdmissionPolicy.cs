    ...
    internal static bool IsSupportedDocumentOrFolderPath(string target, out string? failureCode, out string? reason)
    {
        ...
        switch (category)
        {
            ...
            case OpenTargetPathCategory.DriveRelative:
                ...
                reason = "V1 open_target принимает только absolute local/UNC path; drive-relative target не поддерживается.";
                return false;
            case OpenTargetPathCategory.DeviceStyle:
                failureCode = OpenTargetFailureCodeValues.InvalidRequest;
                reason = "V1 open_target принимает только absolute local/UNC path и не принимает device-style path forms.";
                return false;
            case OpenTargetPathCategory.AbsoluteUri:
                failureCode = OpenTargetFailureCodeValues.InvalidRequest;
                reason = "V1 open_target принимает absolute local/UNC path для document/folder и не принимает URI в этих targetKind.";
                return false;
            default:
                failureCode = OpenTargetFailureCodeValues.InvalidRequest;
                reason = "V1 open_target принимает только absolute local/UNC path для document/folder; relative target не поддерживается.";
                return false;
        }
    }
