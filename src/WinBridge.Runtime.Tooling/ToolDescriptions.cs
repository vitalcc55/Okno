...
public static class ToolDescriptions
{
    ...
    public const string ComputerUseWinTypeTextTool = "Печатает текст в текущий app session. По умолчанию требует focused editable proof; explicit allowFocusedFallback=true требует confirm=true и либо fresh target-local focus proof, либо explicit capture_pixels point из последнего screenshot state для coordinate-confirmed poor-UIA fallback. Coordinate-confirmed path dispatch-ит click+type через SendInput, остаётся verify_needed и не добавляет hidden clipboard fallback. Optional observeAfter=true materialize-ит successorState + screenshot после committed action.";
    public const string ComputerUseWinPressKeyTool = "Нажимает named key literal или modifier combo в текущий app session. Bare printable text сюда не входит и должен идти через type_text. Optional observeAfter=true materialize-ит successorState + screenshot после committed action.";
    ...
    public const string ComputerUseWinScrollTool = "Скроллит app session по elementIndex или point из последнего app state. Optional observeAfter=true materialize-ит successorState + screenshot после committed action.";
    ...
    public const string ComputerUseWinDragTool = "Делает drag gesture в app session по element indices или coordinates из последнего app state. Optional observeAfter=true materialize-ит successorState + screenshot после committed action.";
