...
public static class ToolDescriptions
{
    ...
    public const string ComputerUseWinSetValueTool = "Семантически устанавливает text или number value у конкретного элемента из последнего app state через ValuePattern/RangeValuePattern без hidden typing fallback. Public action result публикует executionFacts для semantic path.";
    public const string ComputerUseWinScrollTool = "Скроллит app session по elementIndex или point из последнего app state. Public action result публикует executionFacts, которые различают semantic UIA path и expected_physical coordinate fallback; optional observeAfter=true materialize-ит successorState + screenshot после committed action.";
    public const string ComputerUseWinPerformSecondaryActionTool = "Выполняет product-owned secondary action над semantic target из последнего app state. Public action result публикует executionFacts для semantic path.";
    public const string ComputerUseWinDragTool = "Делает drag gesture в app session по element indices или coordinates из последнего app state. Public action result публикует executionFacts для expected_physical drag path; optional observeAfter=true materialize-ит successorState + screenshot после committed action.";
