...
public static class ToolContractManifest
{
    ...
    public static IReadOnlyList<ToolDescriptor> All { get; } =
        new[]
        {
            ...
            new ToolDescriptor(ToolNames.WindowsListMonitors, "windows.display", ToolLifecycle.Implemented, ToolSafetyClass.ReadOnly, "Перечисляет активные monitor targets текущей desktop session.", null, null, true),
            ...
            new ToolDescriptor(ToolNames.WindowsActivateWindow, "windows.shell", ToolLifecycle.Implemented, ToolSafetyClass.OsSideEffect, "Восстанавливает окно при необходимости и пытается подтвердить foreground focus.", null, null, true),
