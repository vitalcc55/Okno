...
public static class ToolContractManifest
{
    ...
    public static string ComputerUseWinContractNotes { get; } =
        "Public product profile `computer-use-win` публикует vendor-like operator surface поверх внутреннего Okno engine. В текущем shipped subset в public profile входят `list_apps`, `get_app_state` и `click`; `list_apps` группирует visible window instances по app-level approval identity и публикует nested `windows[]` с opaque `windowId`, а `get_app_state` таргетирует конкретный instance через `windowId` или explicit `hwnd`. `type_text`, `press_key`, `scroll` и `drag` закреплены как следующий глобальный action wave и пока не публикуются как implemented product surface. Низкоуровневые `windows.*` tools остаются внутренним execution substrate и не являются главным product-facing Codex UX.";
