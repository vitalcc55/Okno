...
internal static class ComputerUseWinActionObservability
{
        ...
        ComputerUseWinActionObservabilityContext context)
    {
        ...
        ComputerUseWinActionArtifactPayload artifactPayload = new(
            ...
            DispatchClass: payload.ExecutionFacts?.DispatchClass,
            Executor: payload.ExecutionFacts?.Executor,
            TargetProof: payload.ExecutionFacts?.TargetProof,
            WindowContinuity: payload.ExecutionFacts?.WindowContinuity,
            ForegroundIntegrity: payload.ExecutionFacts?.ForegroundIntegrity,
            PhysicalPointerUsed: payload.ExecutionFacts?.PhysicalPointerUsed,
            PhysicalKeyboardUsed: payload.ExecutionFacts?.PhysicalKeyboardUsed,
            SystemCursorMoved: payload.ExecutionFacts?.SystemCursorMoved,
    ...
    private static Dictionary<string, string?> CreateEventData(
        ...
        new Dictionary<string, string?>(StringComparer.Ordinal)
        {
            ...
            ["dispatch_class"] = payload.ExecutionFacts?.DispatchClass,
            ["executor"] = payload.ExecutionFacts?.Executor,
            ["target_proof"] = payload.ExecutionFacts?.TargetProof,
            ["window_continuity"] = payload.ExecutionFacts?.WindowContinuity,
            ["foreground_integrity"] = payload.ExecutionFacts?.ForegroundIntegrity,
            ["physical_pointer_used"] = payload.ExecutionFacts?.PhysicalPointerUsed.ToString().ToLowerInvariant(),
            ["physical_keyboard_used"] = payload.ExecutionFacts?.PhysicalKeyboardUsed.ToString().ToLowerInvariant(),
            ["system_cursor_moved"] = payload.ExecutionFacts?.SystemCursorMoved.ToString().ToLowerInvariant(),
