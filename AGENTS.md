...
- `src/**/bin`, `tests/**/bin` и `obj` не считать runtime launch surface для integration/smoke: сервер, helper и другие внешние процессы должны запускаться только из staged run bundle под `.tmp/.codex/...`, а не из mutable build cache.
...
- Built-in OpenAI `computer use` не считать replacement для `Okno`: текущий продуктовый путь теперь идёт через public plugin `computer-use-win`, а `Okno` остаётся внутренним Windows-native engine. Не тащи OpenAI-specific contracts в `WinBridge.Runtime` / `WinBridge.Server`, но и не строй отдельный второй runtime ради plugin surface.
- Если меняется plugin install surface (`plugins/computer-use-win`, launcher, install hint, marketplace/install model), acceptance обязан включать proof для cache-installed copy, restart Codex/new thread и минимум один реальный read-only или state-first tool call из fresh-thread materialization path.
...
- Verification-first loop для fast CI уже нормализован: `scripts/bootstrap.ps1` -> `scripts/build.ps1` -> `scripts/test.ps1` -> `scripts/smoke.ps1` -> `scripts/refresh-generated-docs.ps1`; для повседневного локального контура используй `scripts/ci.ps1`, а для полного release/install proof — `scripts/release-verify.ps1`.
