...
- для `capture_pixels` canonical client path копирует `captureReference` из `windows.capture` без ручной сборки geometry, если capture смог опубликовать input-compatible reference; observe metadata `bounds/frameBounds` у capture payload сохраняют обычный public shape с derived `width/height`, а nested `captureReference.bounds/frameBounds` используют edge-only input-compatible `left/top/right/bottom`; тот же `captureReference` несёт target provenance (`hwnd/processId/threadId/className`) той capture surface, из которой были получены pixels, поэтому input fail-close-ится не только на stale geometry, но и на absent/mismatched target provenance; `captureReference.frameBounds` фиксирует WGC acquisition-time frame basis для window captures, raster origin должен быть доказан capture layer как visible-frame origin внутри него, post-capture refresh перечитывает live window snapshot и валидирует frame/raster/DPI drift + stable identity eligibility, иначе copy-through geometry не публикуется успешным observe capture; resize после capture должен возвращать stale failure, а не destructive click;
...
- для `window` capture `bounds` описывает WGC acquisition-time raster/content basis, а `frameBounds` публикует matching frame basis из той же capture transaction; эти observe fields сохраняют обычный `Bounds` wire shape с derived `width/height`, а отдельный nested `captureReference` публикует input-compatible geometry и target provenance для прямого copy-through в `windows.input`, если stable target provenance доступна и подтверждена live refresh; raster origin не синтезируется от frame origin при меньшем WGC content size, post-capture refresh не становится source-of-truth для geometry, drift по frame/raster/DPI fail-closes capture, а absent/unconfirmed captured-target provenance suppresses `captureReference` and fail-closes only capture-derived input.
