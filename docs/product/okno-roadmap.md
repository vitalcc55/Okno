...
| 13 | `windows.launch_process` | явный запуск `.exe` / процесса с отдельной моделью ошибок | `реализовано` | `85%` | `R1-следом` |
| 14 | `windows.open_target` | shell-open файла / URL / document target без смешения с process launch | `реализовано` | `85%` | `R1-следом` |
...
| 20 | proposed `windows.surface_lifecycle` | claim/reconcile/close only owned shell/window/dialog surfaces after `launch_process` / `open_target`; fail-closed на reused unowned surface | `запланировано` | `0%` | `R2-R3` |
| 21 | `artifacts/diagnostics` + scripts + generated docs | retention/cleanup, scenario runner, help/resources, agent-facing docs pack | `частично` | `30%` | `R3` |
| 22 | proposed daemon / overlay / virtual desktop slices | background companion, visualizer, virtual desktop support | `запланировано` | `0%` | `R3-опция` |
...
- не пытаться решать cleanup/teardown reused shell surfaces внутри `windows.open_target`: safe close требует отдельной ownership/lifecycle модели и должен fail-closed, если surface ownership не доказан;
- сдвигать минимальный hardening влево: policy/deny/redaction/dry-run должны появляться до широкого destructive input.

---

## 1.7. Compatibility track: OpenAI shell / skills / computer use

Эта дорожка не меняет ближайший V1 priority order и не является replacement для локального Windows runtime.

Что фиксируем заранее:

- текущий локальный integration path для Codex остаётся `shell + Okno(MCP/plugin) + skills`;
- built-in OpenAI `computer use` рассматривается как будущая compatibility track, а не как немедленная зависимость;
- `windows.input` нужно проектировать vocabulary-compatible с типовым GUI action loop, но без подмены core runtime внешним API;
- отдельный adapter `computer_call -> Okno -> computer_call_output` должен жить поверх `Okno`, а не внутри `WinBridge.Runtime` / `WinBridge.Server`.

Activation criteria для этой дорожки:

- shipped `windows.input` хотя бы в объёме `click`-first;
- зафиксированный action schema contract для `windows.input`;
- сохранение split `windows.launch_process` / `windows.open_target`;
- отдельный architecture/exec-plan source of truth, не смешанный с ближайшим delivery плана `launch -> input -> clipboard`.
...
Сюда же относится преждевременная перестройка продукта вокруг built-in `computer use`: совместимость с внешними agent loops полезна, но не должна менять ближайший delivery order `launch -> input -> clipboard`.
