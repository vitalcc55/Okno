...
| 11 | `plugins/computer-use-win` + poor-UIA `type_text` fallback follow-up | explicit `allowFocusedFallback=true` fallback for poor-UIA text-entry-like targets after screenshot-first navigation: focused path with fresh target-local focus proof, plus coordinate-confirmed `point`/`coordinateSpace` path for top-level-only Qt/custom UI focus, always `confirm=true`, no arbitrary focused-clickable typing, no hidden previous-click reuse, no clipboard default and public `verify_needed` semantics. Repo implementation and helper proof are in Stage 7; cache-installed Telegram product acceptance remains Stage 8. | `частично` | `95%` | `R2-следом` |
...
- poor-UIA text-entry implementation path уже landed в repo как bounded `type_text` fallback slice через explicit `allowFocusedFallback=true` + `confirm=true`: focused path требует fresh target-local focus proof, а coordinate-confirmed path принимает explicit `point`/`coordinateSpace` из последнего screenshot state для top-level-only Qt/custom UI, без hidden previous-click reuse, clipboard default и optimistic `done`; real Telegram/cache-installed product acceptance остаётся Stage 8 gate;
