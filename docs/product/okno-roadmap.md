...
| 11 | `okno.health` + runtime guard layer | reporting-first readiness baseline: desktop session, integrity/UIPI, capture/UIA/wait readiness и conservative blocked summaries для future input/clipboard/launch без hidden enforcement | `реализовано` | `80%` | `R1-следом` |
