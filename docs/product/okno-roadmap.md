...
| 09b | `plugins/computer-use-win` + observation completeness / deep semantic lookup | screenshot-backed `get_app_state` больше не collapses visual observation из-за failed/incomplete UIA preview; public state публикует product-owned `semanticPreview`, stored completeness metadata и warnings, а semantic/physical actions могут reach-ить bounded selector targets beyond compact preview по `AutomationId`/`ControlType`/optional `Name`; deterministic repo proof-smoke и cache-installed proof покрывают этот shipped public contract | `реализовано` | `100%` | `R2-следом` |
...
- observation completeness / deep semantic lookup закрыт как interstitial R2 slice: visual observation и semantic preview readiness теперь разделены, compact preview больше не является единственной semantic reachability surface, а app playbooks не должны claim-ить этот proof за счёт physical-policy slice;
