...
По состоянию на `2026-03-18` репозиторий уже находится не в greenfield-точке, а в состоянии **bootstrap-платформы + реализованного observe/window контура**.
...
- `src/WinBridge.Runtime.Windows.UIA`, `Input`, `Clipboard`, `Waiting` пока существуют как подготовленные seams и deferred tools, а не как реализованные capability-срезы.
...
| 09 | `src/WinBridge.Runtime.Windows.UIA` + `windows.uia_snapshot` | minimal semantic snapshot для explicit/attached/active window | `только_seam` | `15%` | `R1-следом` |
| 10 | `src/WinBridge.Runtime.Waiting` + `windows.wait` | публичный wait tool: window/focus/element/visual conditions | `только_seam` | `20%` | `R1-следом` |
| 11 | `okno.health` + runtime guard layer | готовность среды: desktop session, integrity/UIPI, capture/UIA readiness, unsupported paths | `запланировано` | `15%` | `R1-следом` |
| 12 | `WinBridge.Runtime.Tooling` + `WinBridge.Server` + `WinBridge.Runtime.Diagnostics` | safety baseline: allow/deny flags, dry-run, false-success policy, redaction hooks | `частично` | `50%` | `R1-следом` |
...
Package A для этого этапа закрывает только contract-first groundwork: target policy, typed DTO/seam и tests. Public MCP rollout, runtime service и smoke остаются следующими пакетами.
