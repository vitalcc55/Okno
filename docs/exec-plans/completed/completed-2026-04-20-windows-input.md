...
- request semantics должна быть ещё и типизированной по ролям параметров. `RunId`, `RunRoot`, `ArtifactsRoot` и `AssemblyBaseDirectory` являются context-defining / provenance inputs и должны либо совместно указывать на один и тот же `runId`, либо fail-fast завершаться как внутренне противоречивый explicit request. `PreferredSourceContextName` относится к другой категории: это selection hint внутри уже выбранного context, а не новый источник run identity. Поэтому shared resolver обязан отключать ambient manifest/DLL fallback для такого explicit selection request, но не имеет права из-за него сбрасывать active run context до default `local`.
- typed request semantics не должна переусердствовать с identity derivation. `ArtifactsRoot` и `AssemblyBaseDirectory` остаются canonical provenance sources, а `RunRoot` может быть либо canonical `.tmp/.codex/runs/<run-id>` identity-bearing path, либо arbitrary explicit staging path. Только canonical `RunRoot` разрешено использовать для derivation `runId`; leaf неканонического path не является source of truth для identity и не должен участвовать в inconsistency validation. Отдельно precedence и validation должны оставаться разными фазами: сильный override `ManifestPath` выбирает source-of-truth только после того, как explicit request уже проверен на внутреннюю непротиворечивость, иначе mixed `ManifestPath + RunId/RunRoot/ArtifactsRoot/AssemblyBaseDirectory/PreferredSourceContextName` снова превращаются в silent short-circuit.
- sibling MCP baseline снова консистентен после отдельного Stabilization S0: shared async-state readiness probe больше не падает `EntryPointNotFoundException` внутри `okno.health` / `windows.launch_process` / `windows.open_target`, потому что Win32 `GetCurrentThreadId` / `GetThreadDesktop` / `OpenInputDesktop` / `CloseDesktop` теперь связаны через явный `EntryPoint`, а не через локальные `*Native` method names. За счёт этого verification loop снова подтверждает honest `health/readiness -> gate -> tool result` цепочку без reopening control-plane workstream и без отдельного pre-gate invalid-request hardening в том же цикле.
...
- normal clients copy `captureReference` from that single capture result instead of reconstructing input geometry from observe metadata or synthesizing frame geometry through `windows.wait` / `windows.list_windows`; public capture `bounds` / `frameBounds` remain ordinary observe rectangles with derived `width` / `height`, while `captureReference.bounds` / `captureReference.frameBounds` use the exact edge-only `InputBounds` wire shape and `captureReference.targetIdentity` preserves the captured window provenance needed for fail-closed remap; if capture cannot prove stable target provenance, capture may still return valid observe payload without input-compatible `captureReference`;
- for window WGC captures, `captureReference.bounds.Left/Top` is a proven raster/content origin inside `captureReference.frameBounds`, not a synthetic reuse of the Win32 frame origin when content size is smaller than the frame; post-capture refresh materializes a live window snapshot, validates frame/raster/DPI drift, and gates `captureReference` publication on confirmed stable target identity, but never becomes the authoritative `captureReference` geometry source;
...
- `capture_reference_stale` is raised when any of the following becomes true before dispatch:
  - `captureReference.targetIdentity` is absent, malformed or no longer matches the current live target window by `HWND + ProcessId + ThreadId + ClassName`;
...
| [scripts/common.ps1](../../../scripts/common.ps1) + `scripts/codex/prepare-okno-test-bundle.ps1` + `scripts/codex/resolve-okno-test-bundle.ps1` | build/run isolation control plane | Reuse staged run bundle and `.NET artifacts` output layout; compute one effective execution context for every bundle entry point and for integration harness, derive `runId`/`runRoot` from explicit artifacts provenance when available, select one coherent `server + helper` source context by freshness/provenance, materialize bundle only for that context, and enforce one precedence contract where explicit parameters win over ambient env/session state. |
| [scripts/smoke.ps1](../../../scripts/smoke.ps1) | real proof path | Extend owned helper scenario with click-first proof and artifact/event assertions through the staged run bundle, not direct `bin` launch. |
