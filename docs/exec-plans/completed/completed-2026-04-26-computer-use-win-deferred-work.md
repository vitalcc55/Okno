...
- Post-final review follow-up (`2026-04-26`):
  ...
  - `[confirmed]` third external review wave разделил active problems точнее: (1) observed-state для action-ready coordinate path должен быть строже attached refresh; (2) latest published discovery batch должен переживать следующий follow-up issuance, а не только собственный generation; (3) `list_apps` metadata/manifest должен честно описывать stateful selector issuance. Follow-up fix сделал `ComputerUseWinStoredStateResolver` request-mode-aware, усилил catalog lifetime до protected latest published discovery snapshot и синхронизировал MCP annotations / safety class для `list_apps`.
  - `[confirmed]` final re-review wave нашёл ещё два closure gaps: stale generated `computer-use-win` interface exports и empty publication, не сбрасывающий protected latest discovery snapshot. Follow-up fix прогнал canonical `refresh-generated-docs`, синхронизировал checked-in generated surface и сделал empty discovery publication явным lifecycle transition с reset protected generation.
  - `[confirmed]` fourth external review wave разделил два разных closure gaps: missing capture proof ошибочно классифицировался как `stale_state` до action contract path, а `list_apps` после stateful selector issuance всё ещё публиковал `destructiveHint=false`. Follow-up fix убрал `CaptureReference == null` из continuity proof ownership, поэтому live target без capture proof возвращает `capture_reference_required`, и синхронизировал MCP annotations для `list_apps` на `destructiveHint=true`.
  - `[confirmed]` fifth external review wave нашёл, что empty publication и non-empty replacement snapshot снимали protection, но не делали selectors прошлого snapshot немедленно невалидными: product-validity всё ещё частично зависела от TTL/overflow storage mechanics. Follow-up fix перевёл `TryResolveWindowId` на explicit current-published-snapshot ownership check; old `windowId` теперь возвращает `missing_target` после любого нового `list_apps`, включая empty publication, а follow-up `TryIssue` не разрушает latest published batch.
  - `[confirmed]` sixth external review wave нашёл, что explicit `hwnd` и attached fallback всё ещё публиковали ad hoc `windowId` в public session/approval payload, хотя такой id не принадлежал current published discovery snapshot и сразу падал в `missing_target`. Follow-up fix разделил reusable `PublicWindowId` и internal `ExecutionTargetId`: `TryIssue` теперь переиспользует current published selector при strict match, иначе возвращает target без public selector; public `session.windowId` стал optional, а audit пишет `window_id` только для real selector и отдельный `execution_target_id`.
  - latest post-review verification: targeted public selector RED pack failed as expected `3/3`, затем GREEN `3/3`; combined selector lifecycle pack -> green `6/6`; `ComputerUseWinActionAndProjectionTests|ComputerUseWinArchitectureTests|ComputerUseWinFinalizationTests|ComputerUseWinObservationTests|targeted MCP smoke` -> green `118/118`; runtime exporter/manifest/audit contour -> green `48/48`; install-surface + full MCP smoke contour -> green `49/49`; `scripts/refresh-generated-docs.ps1` completed with zero build warnings/errors; `scripts/codex/verify.ps1` -> green, Runtime `638/638`, Server `280/280`, smoke ok, refresh-generated-docs ok.
  - `[confirmed]` sixth-wave review gate found one remaining audit gap: observed-state completion still materialized a `window_id` key with null value when no public selector was published. Follow-up TDD added audit data tests for no-selector and published-selector branches; builder now omits `window_id` unless `PublicWindowId` is non-blank and always writes `execution_target_id`. Targeted audit/finalization contour -> green `28/28`.
  - latest review gate: tests/failure/docs/generated subagent `Erdos` -> first-pass `approve`, re-review after audit fix `approve`; architecture/contract subagent `Goodall` -> first-pass `changes_requested` on audit `window_id`, re-review after TDD fix `approve`; no confirmed findings remained open.
  - `[confirmed]` seventh external review wave нашёл, что `get_app_state(windowId)` мог публиковать pre-activation `session.windowId` после activation drift: restored/minimized/arranged window менял strict discovery snapshot fields, а payload/stored state/audit всё ещё несли selector, который следующий `get_app_state(windowId)` reject-ил бы. Follow-up fix сделал selector publication phase-aware: handler revalidates any `activation.Window` через catalog-owned strict current-snapshot proof, а non-success activation result без descriptor fail-close-ит public selector before observation/finalizer/audit.
  - seventh-wave review gate: architecture/contract subagent `Chandrasekhar` -> first-pass `approve_with_minor_notes`, final doc-only re-review `approve`; tests/failure/docs subagent `Kant` -> first-pass `changes_requested` on `ambiguous`/`failed-with-window` warning branches, re-review after TDD fix `approve_with_minor_notes`, final doc-only re-review `approve`. Targeted activation RED/GREEN: `1/1`; warning-path RED/GREEN: `2/2`; selector/finalization contour -> green `31/31`; broader computer-use-win contour -> green `123/123`; `scripts/refresh-generated-docs.ps1` -> green; `scripts/codex/verify.ps1` -> green, Runtime `638/638`, Server `285/285`, smoke ok, refresh-generated-docs ok.
- Deferred decisions:
