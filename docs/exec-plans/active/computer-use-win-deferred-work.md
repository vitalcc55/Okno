...
- Post-final review follow-up (`2026-04-26`):
  ...
  - post-review verification: targeted selector lifecycle RED pack failed as expected `2/3`, затем targeted GREEN `3/3`; adjacent selector/surface contours stayed green: `ComputerUseWinActionAndProjectionTests|ComputerUseWinArchitectureTests` -> `88/88`, `ComputerUseWinObservationTests|ComputerUseWinFinalizationTests` -> `26/26`, `ComputerUseWinInstallSurfaceTests` -> `27/27`, `McpProtocolSmokeTests` -> `22/22`. A broad `ComputerUseWin` filter timed out and the first full verify hit one foreground-sensitive smoke failure while stale test-bundle `Okno.Server.exe` processes were still running; after cleanup the isolated smoke passed `1/1` and repeated `scripts/codex/verify.ps1` was green: Runtime `638/638`, Server `278/278`, smoke ok, refresh-generated-docs ok.
  - review gate: architecture/contract subagent `Hegel` -> `approve`; tests/failure/docs/generated subagent `Zeno` -> `approve`; no confirmed findings remained open.
- Deferred decisions:
