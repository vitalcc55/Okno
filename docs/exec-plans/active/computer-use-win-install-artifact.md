...
- Rollback сохраняет usable source до validated canonical restore: corrupt manifest backup fail-close-ится без consuming `$backupRoot`, а legacy/pre-manifest backup без existing manifest не мигрируется по sentinel-list и остаётся rejected recovery source.
