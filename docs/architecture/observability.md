...
5. Для `windows.uia_snapshot` сверить public `structuredContent.artifactPath` с JSON artifact и, если audit sink был здоров, с событием `uia.snapshot.runtime.completed` в `events.jsonl`. При отсутствии runtime event из-за sink failure source of truth остаётся public payload + JSON artifact.
