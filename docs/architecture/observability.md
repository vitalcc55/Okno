...
| `computer_use_win.action.completed` | Для product-owned action lifecycle `computer-use-win` | Safe metadata + ссылка на `computer-use-win/action-*.json` и, если есть, child runtime artifacts | Один итоговый product event на action result после входа в action lifecycle owner. Event и artifact best-effort: `artifact_write` или event sink failure не меняют публичный action result. Payload несёт только safe fields вроде `action_name`, `public_result`, `failure_code`, `runtime_state`, `lifecycle_phase`, `app_id`, `target_mode`, `coordinate_space`, `confirmation_required`, `confirmed`, `risk_class`, `dispatch_path`, `text_length`, `text_bucket`, `contains_newline`, `whitespace_only`, `value_kind`, `value_length`, `value_bucket`, `scroll_direction`, `scroll_amount_bucket`, `scroll_unit`, `semantic_scroll_supported`, `fallback_used`, `semantic_action_kind`, `context_menu_path_used`, `source_mode`, `destination_mode`, `path_point_count_bucket`, `coordinate_fallback_used`, `observe_after_requested`, `successor_state_available`, `successor_state_failure_code`, `refresh_state_recommended`, `verify_status`, `artifact_path`, `child_artifact_paths`, `failure_stage`, `exception_type`. Focused `type_text` fallback использует `target_mode=focused_fallback` / `element_focused_fallback` и `dispatch_path=win32_sendinput_unicode`; coordinate-confirmed `type_text` fallback использует `target_mode=coordinate_confirmed_fallback`, `risk_class=coordinate_confirmed_text_fallback`, `coordinate_space=capture_pixels` и `dispatch_path=capture_pixels_text_input`. Raw `stateToken`, raw typed text, raw `set_value`, raw key literals/combos, raw point payload, raw successor observation failure reason и raw exception message intentionally не записываются; public `successorStateFailure.reason` строится только через product-owned failure mapper. |
