...
- `artifacts/diagnostics/<run_id>/wait/visual/<visual_wait_artifact>.png` — baseline/current PNG-доказательства для shipped `visual_changed`.
...
| `wait.runtime.completed` | Для `windows.wait` runtime path | Typed metadata + ссылка на wait JSON artifact и visual artifacts, если condition = `visual_changed` | Один итоговый runtime event на один wait call; payload включает `condition`, `status`, `target_source`, `target_failure_code`, `attempt_count`, `elapsed_ms`, `artifact_path`, `matched_text_source` и `diagnostic_artifact_path`; для internal `runtime_unhandled` / `tool_boundary_unhandled` / `artifact_write` failures event дополнительно сохраняет `exception_type` и `exception_message`, без per-tick spam |
...
5. Для `windows.wait` сверить public `structuredContent.artifactPath` с JSON artifact и событием `wait.runtime.completed` в `events.jsonl`; для `visual_changed` дополнительно проверить referenced baseline/current PNG artifacts. Для `runtime_unhandled` и `tool_boundary_unhandled` internal `failure_diagnostics` доступны и в wait artifact, и в runtime event; для `artifact_write` wait artifact уже отсутствует по определению, поэтому расследование идёт только по `wait.runtime.completed` с `failure_stage = artifact_write`, `exception_type` и `exception_message`.
6. Для быстрого доступа к последним артефактам использовать `powershell -File scripts/investigate.ps1`.
