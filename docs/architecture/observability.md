...
| `wait.runtime.completed` | Для `windows.wait` runtime path | Typed metadata + ссылка на wait JSON artifact и visual artifacts, если condition = `visual_changed` | Один итоговый runtime event на один wait call; payload включает `condition`, `status`, `target_source`, `target_failure_code`, `attempt_count`, `elapsed_ms`, `artifact_path`, `matched_text_source` и `diagnostic_artifact_path`; для internal `runtime_unhandled` / `artifact_write` failures event дополнительно сохраняет `exception_type` и `exception_message`, без per-tick spam |
...
5. Для `windows.wait` сверить public `structuredContent.artifactPath` с JSON artifact и событием `wait.runtime.completed` в `events.jsonl`; для `visual_changed` дополнительно проверить referenced baseline/current PNG artifacts. Для `runtime_unhandled` internal `failure_diagnostics` доступны и в wait artifact, и в runtime event; для `artifact_write` wait artifact уже отсутствует по определению, поэтому расследование идёт только по `wait.runtime.completed` с `failure_stage = artifact_write`, `exception_type` и `exception_message`.
