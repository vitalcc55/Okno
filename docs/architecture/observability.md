...
- `artifacts/diagnostics/<run_id>/computer-use-win/action-*.json` — JSON-evidence для product action lifecycle `computer-use-win`: safe action summary, public result, failure stage и ссылки на child runtime artifacts без raw `stateToken`, raw text/value/key или raw exception message.
...
Рядом с broad smoke теперь shipped и более узкий companion proof:
`scripts/computer-use-win-physical-policy-proof-smoke.ps1` materialize-ит
helper-backed real-STDIO report для phase-1 `computer-use-win` executionFacts
и пишет отдельные `report.json` / `summary.md` в
`artifacts/smoke/computer-use-win-physical-policy-phase-1/<run_id>/`.
...
Current shipped phase-1 state для `computer-use-win` observability теперь уже
поднимает execution truth выше старых partial markers `dispatch_path`,
`risk_class`, `fallback_used`, `observe_after_requested`,
`successor_state_available` и `successor_state_failure_code`.

Action result, product event и action artifact теперь materialize-ят один и тот
же public `executionFacts` layer:

- `dispatchClass` = `semantic` / `expected_physical` / `fallback_physical`;
- `executor`;
- `targetProof`;
...
Важно: это всё ещё не новый public tool-zoo и не обещание точного `UIPI` или
`foregroundChanged` diagnosis. Цель phase-1 — сделать текущий
action/result/audit surface честно proof-weighted, а не спрятать physical
dispatch за старым partial telemetry.

## Каналы и anti-noise rules

| Канал | Всегда включён | Что пишет | Anti-noise правило |
| --- | --- | --- | --- |
| `tool.invocation.started/completed` | Да | Старт/завершение каждого MCP tool call, sanitized `request_summary`, internal `gate_*` markers и redaction markers при наличии | Не пишем внутренние step-by-step сообщения по `list_windows`/`attach`; public payload-shaped поля и internal gate metadata не должны конфликтовать по именам. Для `computer-use-win` completion trail теперь идёт через safe builders: допустимы `runtime_state`, `app_id`, public `window_id` когда reusable selector действительно опубликован, internal `execution_target_id`, `state_token_present`, `public_reason` и artifact hints, но raw `stateToken`, raw low-level `reason` и другие sensitive per-handler fields не должны materialize-иться в `events.jsonl`. |
...
| `input.runtime.completed` | Для factual `windows.input` runtime path | Typed metadata + ссылка на input JSON artifact в diagnostics run directory | Один итоговый runtime event на один factual runtime result после входа в `IInputService`, если audit sink доступен; pre-gate invalid/blocked/needs-confirmation paths остаются на `tool.invocation.*` audit trail и не materialize-ят runtime artifact/event. Payload включает `status`, `decision`, `result_mode`, `failure_code`, `target_hwnd`, `target_source`, `completed_action_count`, `failed_action_index`, `action_types`, `coordinate_spaces`, `artifact_path`, `failure_stage`, `exception_type` и `committed_side_effect_evidence`. Помимо click-first markers `click_dispatch_clean_failure`, `click_dispatch_partial_compensated` и `click_dispatch_partial_uncompensated`, evidence теперь также остаётся phase-aware для multi-step gestures: clean failure, cancellation, unexpected failure и partial second-tap failure after `double_click` больше не выглядят как cursor-move-only или generic partial path, а `artifact_write` fallback сохраняет тот же factual committed marker вместо деградации до stage-only evidence. Keyboard/text paths по-прежнему отдельно различают `keypress_dispatch_committed_failure` / `keyboard_dispatch_committed_before_failure` и `text_dispatch_committed_failure` / `text_dispatch_committed_before_failure`, чтобы partial `SendInput` не выглядел как clean no-op. Raw `exception_message`, raw typed text, `key`/`keys` и future keyboard payloads intentionally не попадают ни в event, ни в artifact. `artifact_write` и event-write failures не downcast-ят factual input result. |
| `computer_use_win.action.completed` | Для product-owned action lifecycle `computer-use-win` | Safe metadata + ссылка на `computer-use-win/action-*.json` и, если есть, child runtime artifacts | Один итоговый product event на action result после входа в action lifecycle owner. Event и artifact best-effort: `artifact_write` или event sink failure не меняют публичный action result. Payload несёт только safe fields вроде `action_name`, `public_result`, `failure_code`, `dispatch_class`, `executor`, `target_proof`, `window_continuity`, `foreground_integrity`, `physical_pointer_used`, `physical_keyboard_used`, `system_cursor_moved`, `runtime_state`, `lifecycle_phase`, `app_id`, `target_mode`, `coordinate_space`, `confirmation_required`, `confirmed`, `risk_class`, `dispatch_path`, `text_length`, `text_bucket`, `contains_newline`, `whitespace_only`, `value_kind`, `value_length`, `value_bucket`, `scroll_direction`, `scroll_amount_bucket`, `scroll_unit`, `semantic_scroll_supported`, `fallback_used`, `semantic_action_kind`, `context_menu_path_used`, `source_mode`, `destination_mode`, `path_point_count_bucket`, `coordinate_fallback_used`, `observe_after_requested`, `successor_state_available`, `successor_state_failure_code`, `refresh_state_recommended`, `verify_status`, `artifact_path`, `child_artifact_paths`, `failure_stage`, `exception_type`. Focused `type_text` fallback использует `target_mode=focused_fallback` / `element_focused_fallback`, `dispatch_path=win32_sendinput_unicode` и `dispatch_class=fallback_physical`; coordinate-confirmed `type_text` fallback использует `target_mode=coordinate_confirmed_fallback`, `risk_class=coordinate_confirmed_text_fallback`, `coordinate_space=capture_pixels`, `dispatch_path=capture_pixels_text_input` и `dispatch_class=fallback_physical`. Raw `stateToken`, raw typed text, raw `set_value`, raw key literals/combos, raw point payload, raw successor observation failure reason и raw exception message intentionally не записываются; public `successorStateFailure.reason` строится только через product-owned failure mapper. |
...
2. Для phase-1 action-policy проверки дополнительно прогнать `powershell -File scripts/computer-use-win-physical-policy-proof-smoke.ps1`.
3. Открыть `artifacts/smoke/<run_id>/summary.md` и проверить readiness digest из `okno.health`.
4. Для phase-1 proof открыть `artifacts/smoke/computer-use-win-physical-policy-phase-1/<run_id>/summary.md` и соседний `report.json`.
5. При необходимости посмотреть соседний `report.json`, `artifacts/diagnostics/<run_id>/summary.md`, `artifacts/diagnostics/<run_id>/captures/`, `artifacts/diagnostics/<run_id>/computer-use-win/`, `artifacts/diagnostics/<run_id>/launch/`, `artifacts/diagnostics/<run_id>/uia/`, `artifacts/diagnostics/<run_id>/wait/` и `artifacts/diagnostics/<run_id>/input/`.
6. Для `windows.launch_process` сверить public `structuredContent.artifactPath` с JSON artifact и, если audit sink был здоров, с событием `launch.runtime.completed` в `events.jsonl`; если runtime event показывает `failure_stage=artifact_write`, launch artifact ожидаемо отсутствует и расследование идёт по safe markers `failure_stage` / `exception_type` без raw `exception_message`. При отсутствии runtime event из-за sink failure source of truth остаётся public payload + launch artifact. Для dry-run smoke допускается дополнительная проверка internal marker `launch.preview.completed`, но только как best-effort proof signal и не как public contract.
7. Для `windows.open_target` сверить public `structuredContent.artifactPath` с JSON artifact `launch/open-target-*.json` и, если audit sink был здоров, с событием `open_target.runtime.completed` в `events.jsonl`; для folder dry-run smoke допустима дополнительная проверка internal marker `open_target.preview.completed`, но только как best-effort proof signal и не как public contract. При отсутствии runtime event из-за sink failure source of truth остаётся public payload + open-target artifact.
8. Для `windows.input` сверить public `structuredContent.artifactPath` с JSON artifact `input/input-*.json` и, если audit sink был здоров, с событием `input.runtime.completed` в `events.jsonl`; при failed dispatch обязательно проверить `failure_stage` и `committed_side_effect_evidence`, чтобы clean click failure, partial pointer dispatch, committed keypress failure, committed text failure и multi-step `double_click` partial state не выглядели одинаково. Для multi-step `double_click` это означает, что second-tap partial paths уже не должны публиковаться как generic `partial_dispatch_*`, если factual phase знает больше. Если runtime event показывает `failure_stage=artifact_write`, input artifact ожидаемо отсутствует, но `committed_side_effect_evidence` всё равно должен оставаться factual; расследование идёт по safe markers `failure_stage` / `exception_type` без raw `exception_message`. При отсутствии runtime event из-за sink failure source of truth остаётся public payload + input artifact.
9. Для `computer-use-win` action lifecycle сверить `tool.invocation.completed` с product event `computer_use_win.action.completed` и JSON artifact `computer-use-win/action-*.json`. Для `type_text`, `set_value`, `scroll`, `perform_secondary_action` и `drag` дополнительно проверить safe telemetry buckets (`text_length`/`text_bucket`/`contains_newline`/`whitespace_only`, `value_kind`/`value_length`/`value_bucket`, `scroll_direction`/`scroll_amount_bucket`/`scroll_unit`/`semantic_scroll_supported`/`fallback_used`, `semantic_action_kind`/`fallback_used`/`context_menu_path_used` или `source_mode`/`destination_mode`/`path_point_count_bucket`/`coordinate_fallback_used`) вместо raw payload. Phase-1 result truth теперь дополнительно проверяется через public `executionFacts` и matching event/artifact fields: `dispatch_class`, `executor`, `target_proof`, `window_continuity`, `foreground_integrity`, `physical_pointer_used`, `physical_keyboard_used` и `system_cursor_moved`. Для focused `type_text` fallback расследование идёт по `fallback_used`, `target_mode=focused_fallback` / `element_focused_fallback`, `dispatch_path=win32_sendinput_unicode` и `dispatch_class=fallback_physical`, а не по raw text. Для coordinate-confirmed `type_text` fallback дополнительно проверить `target_mode=coordinate_confirmed_fallback`, `risk_class=coordinate_confirmed_text_fallback`, `dispatch_path=capture_pixels_text_input`, `coordinate_space=capture_pixels`, `capture_reference_present` и `dispatch_class=fallback_physical`; raw point в product action event/artifact не пишется. Для `observeAfter=true` проверить `observe_after_requested`, `successor_state_available`, optional `successor_state_failure_code`, child capture artifact и наличие post-action image content в public tool result; failed successor observe остаётся advisory и не переписывает committed top-level action outcome. Если product artifact write не удался, публичный action payload остаётся source of truth, а investigation идёт по `failure_stage=artifact_write` / `exception_type` в product event, если event sink был доступен.
10. Для `windows.uia_snapshot` сверить public `structuredContent.artifactPath` с JSON artifact и, если audit sink был здоров, с событием `uia.snapshot.runtime.completed` в `events.jsonl`. При отсутствии runtime event из-за sink failure source of truth остаётся public payload + JSON artifact.
...
12. Для `okno.health` не искать `artifactPath`: отсутствие dedicated health artifact/event в этом rollout является ожидаемым contract.
13. Для быстрого доступа к последним артефактам использовать `powershell -File scripts/investigate.ps1`.
