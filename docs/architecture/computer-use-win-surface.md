...
Следующая стратегическая корректировка после shipped screenshot-first
hardening тоже должна быть зафиксирована явно:

- `computer-use-win` не должен строиться вокруг идеи “второго системного
  курсора”;
- `computer-use-win` не должен принимать `UIA-first` как универсальную
  стратегию для всех target apps;
- для weak-semantic / poor-UIA targets physical mouse/keyboard path уже
  является допустимым first-class execution mode;
- physical path при этом не должен становиться casual: он обязан быть
  явным, измеряемым, policy-controlled и подтверждаемым successor observation.
...
- `list_apps` больше не является pure read-only observation hint: вызов публикует latest discovery snapshot и обновляет bounded server-side catalog, от которого зависит следующий `get_app_state(windowId)`.
- repeated unchanged `list_apps` snapshots переиспользуют прежний runtime-owned `windowId` только при strict `MatchesDiscoverySelector` proof; это уменьшает selector churn без добавления public `hwnd + processId` selector.
- materially changed или replacement snapshot не переиспользует старый selector: старые `windowId` больше не являются product-valid selectors после нового incompatible `list_apps`, включая empty publication, даже если storage entry ещё живёт до TTL/overflow cleanup.
...
text/input/edit/query/search-box hint), а coordinate-confirmed branch требует
explicit `point` из последнего app state в `capture_pixels` coordinate space
(`coordinateSpace` можно не передавать, потому что default уже
`capture_pixels`; `screen` для этой typing ветки reject-ится).
Coordinate-confirmed branch dispatch-ит coordinate click + text в одном
SendInput batch, требует capture proof и bounds check для `capture_pixels`, не
...
переписывания action outcome. Successor observe re-resolve-ит post-action live
...
- text entry без доказанного editable UIA proof теперь доступен только через
  explicit `allowFocusedFallback=true` + `confirm=true`: либо fresh focus proof
  для text-entry-like focused target candidate, либо coordinate-confirmed
  `capture_pixels` point path с click+type dispatch, без raw screen-coordinate
  typing, clipboard default и с честным `verify_needed`, а не fake semantic
  success;
...
- next strategic workstream теперь шире обычного approvals-only hardening:
  `computer-use-win physical execution policy hardening` должен собрать в один
  слой execution facts, risky physical confirmation, user-interference rules и
  общую physical-input policy для screenshot-first poor-UIA operation;
- approvals/playbooks: app approval state, risky action confirmation и
  app-specific playbooks остаются связанным, но отдельным safety/UX
  направлением. Advisory app instructions уже существуют, но app capability
  memory/profile layer пока не shipped и не должен подменять execution policy;
- `region_capture` остаётся следующим visual slice после policy hardening:
  он полезен для low-cost visual verification и future OCR bridge, но не
  заменяет execution facts и physical-input policy;
...
- action layer должен оставаться маленьким и semantically clear: `set_value`
  ...
  входят;
- physical execution для weak-semantic targets не должен быть скрытым. Проекту
  нужен явный technical fact layer, который хотя бы различает semantic path,
  expected physical path и fallback physical path, а также говорит, двигался
  ли системный курсор, использовался ли physical keyboard stream и был ли
  successor observation материализован;
