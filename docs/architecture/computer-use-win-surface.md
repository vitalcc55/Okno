...
explicit `point` из последнего app state и optional `coordinateSpace`
(`capture_pixels` по умолчанию). Coordinate-confirmed branch dispatch-ит
coordinate click + text в одном SendInput batch, требует capture proof и bounds
check для `capture_pixels`, не использует hidden previous-click reuse и не
притворяется, что появился child/caret proof. Оба fallback path остаются
...
- text entry без доказанного editable UIA proof теперь доступен только через
  ...
  `point`/`coordinateSpace` path с click+type dispatch, без clipboard default и
  с честным `verify_needed`, а не fake semantic success;
...
- approvals/playbooks: app approval state, risky action confirmation и
  app-specific playbooks остаются отдельным safety/UX workstream, а не частью
  selector или successor-state semantics;
