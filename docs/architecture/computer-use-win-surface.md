...
- click activation failure после уже принятого `stateToken` не должен materialize-иться как `blocked_target`: policy block означает только intentional deny, а missing live target, свернутое окно и неподтверждённый foreground должны оставаться retriable activation/state failures (`missing_target`, `target_minimized`, `target_not_foreground`) с phase `AfterActivationBeforeDispatch`;
