...
| `set_value` | `os_side_effect` | — | Семантически устанавливает text или number value у конкретного элемента из последнего app state через ValuePattern/RangeValuePattern без hidden typing fallback. Public action result публикует executionFacts для semantic path. |
...
| `scroll` | `os_side_effect` | — | Скроллит app session по elementIndex или point из последнего app state. Public action result публикует executionFacts, которые различают semantic UIA path и expected_physical coordinate fallback; optional observeAfter=true materialize-ит successorState + screenshot после committed action. |
| `perform_secondary_action` | `os_side_effect` | — | Выполняет product-owned secondary action над semantic target из последнего app state. Public action result публикует executionFacts для semantic path. |
| `drag` | `os_side_effect` | — | Делает drag gesture в app session по element indices или coordinates из последнего app state. Public action result публикует executionFacts для expected_physical drag path; optional observeAfter=true materialize-ит successorState + screenshot после committed action. |
