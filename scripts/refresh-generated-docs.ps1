...
function New-CommandsMarkdown {
    ...
    $lines = @(
        ...
        '| `dotnet run --project src/WinBridge.Setup.Cli/WinBridge.Setup.Cli.csproj -- runtime install --descriptor-path <path> --json` | install a shared per-user `computer-use-win` runtime into the canonical Codex-home runtime store and mark it current |',
        '| `dotnet run --project src/WinBridge.Setup.Cli/WinBridge.Setup.Cli.csproj -- runtime status --descriptor-path <path> --json` | inspect the current shared per-user runtime store state and descriptor compatibility |',
        ...
        '| `powershell -ExecutionPolicy Bypass -File scripts/codex/package-computer-use-win-runtime-release.ps1 -Version <semver> -Rid win-x64` | package a versioned standalone `computer-use-win` runtime release zip plus SHA256SUMS without mutating the plugin cache |',
        '| `powershell -ExecutionPolicy Bypass -File scripts/codex/package-computer-use-win-plugin-release.ps1 -Version <semver>` | package a versioned thin `computer-use-win` plugin bundle zip plus SHA256SUMS without embedding the runtime directory |',
        '| `powershell -ExecutionPolicy Bypass -File scripts/codex/package-computer-use-win-setup-cli-payload.ps1 -Version <semver> -Rid win-x64` | package the headless setup CLI payload zip plus SHA256SUMS for installer-first distribution |',
        '| `powershell -ExecutionPolicy Bypass -File scripts/codex/package-okno-setup-app-release.ps1 -Version <semver> -Rid win-x64` | package the unsigned WinUI 3 `Okno Setup.exe` RC zip plus SHA256SUMS for installer-first distribution |',
        '| `powershell -ExecutionPolicy Bypass -File scripts/codex/install-computer-use-win.ps1 -Mode codex|runtime-only -PayloadArchivePath <zip> -DescriptorPath <path>` | thin PowerShell bootstrap installer that runs the packaged setup CLI without repo checkout |',
