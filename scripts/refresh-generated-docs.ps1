...
function New-CommandsMarkdown {
    ...
    $lines = @(
        ...
        '| `powershell -ExecutionPolicy Bypass -File scripts/codex/package-okno-setup-app-release.ps1 -Version <semver> -Rid win-x64 -RuntimePackagingResultPath <path>` | package the unsigned WinUI 3 `Okno Setup.exe` RC zip plus SHA256SUMS for installer-first distribution, embedding the canonical runtime descriptor proven by the runtime packaging result |',
